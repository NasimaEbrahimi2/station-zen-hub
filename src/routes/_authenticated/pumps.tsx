...
import { getOverview, getRefills, recordRefill } from "@/lib/station.functions";
...
function PumpsPage() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl space-y-6">
      <div>
        <h1 className="text-2xl md:text-3xl font-semibold tracking-tight">Pumps</h1>
        <p className="text-sm text-muted-foreground mt-1">Refill pumps from tank ({fmtLiters(tankVol)} available).</p>
      </div>
            ...
            <Card key={p.id}>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Capacity {fmtLiters(Number(p.capacity))}</p>
                    </div>
                  </div>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <Stat k="Current" v={fmtLiters(Number(p.current_volume))} />
                  <Stat k="Sold total" v={fmtLiters(Number(p.total_sold))} />
                  <Stat k="Revenue" v={fmtMoney(Number(p.total_revenue), currency)} />
                  <Stat k="Available" v={fmtLiters(Number(p.capacity) - Number(p.current_volume))} />
                </div>
                {!open ? (
                  <Button size="sm" variant="secondary" className="w-full" onClick={() => setOpenId(p.id)}>Refill from tank</Button>
                ) : (
                  <form
